...
func (c *matchingEngineClient) StreamTrades(ctx context.Context, in *StreamTradesRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[TradeEvent], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
...
type MatchingEngine_StreamTradesClient = grpc.ServerStreamingClient[TradeEvent]
...
func (UnimplementedMatchingEngineServer) StreamTrades(*StreamTradesRequest, grpc.ServerStreamingServer[TradeEvent]) error {
...
func _MatchingEngine_StreamTrades_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(MatchingEngineServer).StreamTrades(m, &grpc.GenericServerStream[StreamTradesRequest, TradeEvent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type MatchingEngine_StreamTradesServer = grpc.ServerStreamingServer[TradeEvent]
